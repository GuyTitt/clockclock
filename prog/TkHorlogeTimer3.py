...
class Aiguille():
    def __init__(self,zoneAffichage,thetaDep=0,hChangement=None,duréeChangement=None,
                 thetaArrivée=None,xAxe=0,yAxe=0,longueur=0,épaisseur=1,couleur="#000000"):
    ...
    def trace(self):
        self.aig=self.zoneAffichage.create_line(self.xAxe,self.yAxe,
                           self.xAxe+self.longueur*math.cos(self.thetaActuel),self.yAxe+self.longueur*math.sin(self.thetaActuel),
                           fill=self.couleur,width=self.épaisseur,capstyle='round')
    ...
    def deplace(self):
        self.zoneAffichage.coords(self.aig,
                     self.xAxe,self.yAxe,
                     self.xAxe+self.longueur*math.cos(self.thetaActuel),self.yAxe+self.longueur*math.sin(self.thetaActuel))
    ...
    def glisse(self,theta):
        self.zoneAffichage.coords(self.aig,
                     self.xAxe,self.yAxe,
                     self.xAxe+self.longueur*math.cos(theta),self.yAxe+self.longueur*math.sin(theta))
        self.thetaActuel=theta
        
        
def creeMonAiguille(cnv,listePositions,xAxe,yAxe,longueur,épaisseur,couleur,
                     positionHorizontaleBouton,positionVerticaleBouton,largeurBouton,hauteurBouton):
    aiguille=Aiguille(cnv,thetaDep=0, xAxe=xAxe,yAxe=yAxe,
                      longueur=longueur,épaisseur=épaisseur,couleur=couleur)
    """
...
def rendPosition(a,i,j):
    chiffres={0:(((0,6,6),(2,2,0)),((4,6,6),(2,2,4))),#chiffres[numero][colonne][aiguille][positionV]
...
#        MyTimer.__init__(self, 1.0, self.turn)
        Tk.Canvas.__init__(self, parent, width=tailleHorloge+espacementHorloge, height=tailleHorloge+espacementHorloge)

        self.create_oval(espacementHorloge,espacementHorloge,tailleHorloge+espacementHorloge,tailleHorloge+espacementHorloge, width = 2,
                         fill = "#888888")
#        for i in range(60):
...
#        self._minute = self.create_line(0, 0, 0, 0, width = 2, fill = "blue")
        ...
        self.ligne=ligne
        self._minute=creeMonAiguille(self,rendPosition(0,colonne,ligne),
                                     int(tailleHorloge/2)+espacementHorloge,int(tailleHorloge/2)+espacementHorloge,
                                     longueurAiguille,largeurAiguille,couleurAiguille[0],
                                     espacementHorloge,espacementHorloge+tailleHorloge-tailleBouton,
                                     tailleBouton,tailleBouton)
        self._hour = creeMonAiguille(self,rendPosition(1,colonne,ligne),
                                     int(tailleHorloge/2)+espacementHorloge,int(tailleHorloge/2)+espacementHorloge,
                                     longueurAiguille,largeurAiguille,couleurAiguille[1],
                                     espacementHorloge,espacementHorloge+tailleHorloge-tailleBouton,
                                     tailleBouton,tailleBouton)
...
def deplace(actu, nouv ,duréeDéplacement, echantillon, echantillonnage):
    for hor in listeHorloge:
        nouvellepositionAiguille=rendPosition(1,hor.colonne,hor.ligne)[nouv[int(hor.colonne/2)]]
        if hor.colonne==7 and hor.ligne==0:
            print (f"Déplacement de {hor._hour.thetaActuel} [{hor._hour.thetaActuel}] vers {nouvellepositionAiguille}",end=" et ")
        if hor._hour.thetaDépart!=nouvellepositionAiguille:
            hor._hour.glisse(hor._hour.thetaDépart+(nouvellepositionAiguille-hor._hour.thetaDépart)*echantillon*echantillonnage/duréeDéplacement)
        nouvellepositionAiguille=rendPosition(0,hor.colonne,hor.ligne)[nouv[int(hor.colonne/2)]]
        if hor.colonne==7 and hor.ligne==0:
        ...
        if hor._minute.thetaDépart!=nouvellepositionAiguille:
            hor._minute.glisse(hor._minute.thetaDépart+(nouvellepositionAiguille-hor._minute.thetaDépart)*echantillon*echantillonnage/duréeDéplacement)
...
def maj():
    ...
    if int(maintenant[5])>debutDéplacement :
        ...
        deplace(positionsActuelle, positionsDemandées, 60-debutDéplacement ,échantillon, échantillonnage)
    else:
        for hor in listeHorloge:
